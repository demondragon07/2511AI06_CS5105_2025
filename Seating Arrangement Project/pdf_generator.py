...
class AttendancePDFGenerator:
    ...
    def draw_header(self):
        ...
        self.y_curr -= (header_height + 10) 

    def draw_student_block(self, x_pos, y_pos, student: Student):
        """Draws student block with separator line."""
        c = self.c
        ...
        img_x = x_pos + 5
        img_y = y_pos + (ROW_HEIGHT - PHOTO_HEIGHT) / 2 + 5
        ...
        if final_photo_path:
            try:
                ...
                c.drawString(img_x, img_y + 25, "Img Error")
        else:
            c.setFont("Helvetica-Oblique", 8)
            c.drawString(img_x + 5, img_y + 25, "No Image") 
            c.drawString(img_x + 5, img_y + 15, "Available")

        separator_x = x_pos + PHOTO_WIDTH + 10
        c.line(separator_x, y_pos, separator_x, y_pos + ROW_HEIGHT)

        text_x = separator_x + 5 
        
        text_y_start = y_pos + ROW_HEIGHT - 20 
        c.setFont("Helvetica-Bold", 10)
        safe_name = (student.name[:14] + '..') if len(student.name) > 16 else student.name
        c.drawString(text_x, text_y_start, safe_name)
        
        c.setFont("Helvetica", 10)
        c.drawString(text_x, text_y_start - 15, f"Roll: {student.roll}")
        
        sign_y = y_pos + 15 
        c.setFont("Helvetica", 9) 
        c.drawString(text_x, sign_y, "Sign: ___________")
    ...
    def draw_footer_table(self):
        ...
        col_w_name = 200
        col_w_sign = 200
        table_width = col_w_sno + col_w_name + col_w_sign
        table_start_x = (PAGE_WIDTH - table_width) / 2
        
        self.y_curr -= row_h
        c.rect(table_start_x, self.y_curr, table_width, row_h)
